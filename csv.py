...
def readCsvLine(f, width = -1, remove_blank_lines = True):
	...
	while True:
		...
		#print line
		assert(line[-1] != ',')
		return misc.padRight(getCsvLine(line), width, '')

def readCsvGen(f, width = -1, remove_blank_lines = True): 
	""" Generator to read a CSV file as a list of list of string """
	while True:
		data = readCsvLine(f, width, remove_blank_lines)
		if not data:
			break
		yield data
...
def readCsv(filename, has_header = True): 
	...
	if has_header:
		...
		width = len(header)
	print 'width', width
	entries = [row for row in readCsvGen(f, width)]
	for row in entries:
		assert(len(row) == width )
	f.close()
	print 'readCsv(%s)' % filename, '=>', len(entries), 'rows x', width, 'columns'
